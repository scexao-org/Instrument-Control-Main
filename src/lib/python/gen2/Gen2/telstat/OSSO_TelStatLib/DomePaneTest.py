...
# DomePaneTest.py -- Arne Grimstrup -- 2003-10-01
# copied shameless from PointingPaneTest.py -- Bruce Bon -- 2003-09-29 12:05

# Test program for DomePane.py
...
def SigHandler(signum, frame):
    ...
    print "DomePaneTest.SigHandler() called, signum = ",	\
	    signum,".\n"
    EXITCODE = signum
    EndProc()
...
def EndProc():
    """Exit function."""
    print "DomePaneTest.EndProc() called, EXITCODE = ",	\
	    EXITCODE,".\n"
    root.destroy()
    sys.exit( EXITCODE )
...
def RootRefresh():
    global dbgcnt
    print "DomePaneTest.RootRefresh() called, time = %.6f" % time.time()
    StatIO_service()
...
root["bg"] = "black"
root.iconname( 'DomePaneTest' )
root.title( 'DomePaneTest' )
