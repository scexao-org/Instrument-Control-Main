...
class TaskAppView(object):
    ...
    def __init__(self, logger, usethread=True, ev_quit=None):
        ...
        dic = { "gtk_main_quit" : self.destory, 
                "on_button_monitor_clicked" : self.close_by_button, 
                "on_button_clear_clicked" : self.clear_tree,
    ...
    def __init_treeview(self):
        ...
        self.treelist = gtk.TreeStore(str, str, str, str, str, str, str, str, str )
        treeview.set_model(self.treelist)
    
        # colunm labels 
        for (col,num) in [('task', 0), ('status',1), ('task start',2), ('task end',3), ('cmd desc',4), ('cmd start',5), ('cmd ack',6), ('est time',7), ('cmd end',8)]:
            treeview.append_column(gtk.TreeViewColumn(col, gtk.CellRendererText(), markup=num))

        # Allow drag and drop reordering of rows
        #treeview.set_reorderable(True)

    def clear_tree(self, obj):
        print 'clear clicked'
        self.treelist.clear()

    def close_by_button(self, obj):
        print 'Button'
        gtk.main_quit() 
...
#        return "%02d:%02d:%02d" %(int(hour),int(min),int(sec))
    ...
    def _get_cmd_desc(self, tree, task_status):
               
        cmd_str = str(task_status.get('cmd_str'))
        cmd_desc =str(task_status.get('cmd_desc'))
        print "CMD INFO <%s>" %(cmd_str + ':' + cmd_desc)
        self.task_view.treelist.set(tree, 4, cmd_str + ' #' + cmd_desc)
         
    def _get_cmd_time(self, tree, task_status):
             
        cmd_time = task_status.get('cmd_time')
        cmd_time=self._convTime(cmd_time)
                    
        print "CMD TIME <%s>" %(cmd_time)
        self.task_view.treelist.set(tree, 5, cmd_time)
  
        
    def _get_ack_info(self, tree, task_status):
        
                           
        ack_time = task_status.get('ack_time')
        ack_time=self._convTime(ack_time)
                      
        if not task_status.get('ack_result')==0:
             ack_time='<span background="red">ack error:%s</span>' %(str(task_status.get('ack_result')))
            
        est_time =task_status.get('est_time')
        if est_time:
            est_time=self._convTime(est_time)
        else:
            est_time='None'         
              
        self.task_view.treelist.set(tree, 6, ack_time )
        self.task_view.treelist.set(tree, 7, est_time)
          
        
     
    def _get_end_time(self, tree, task_status):
        
        end_time = task_status.get('end_time')
        end_time = self._convTime(end_time)
        
        if not task_status.get('end_result') == 0:
            end_time='<span background="red">end error:%d</span>' %(task_status.get('end_result'))
        
        print "CMD END <%s>" %(end_time)
        self.task_view.treelist.set(tree, 8, end_time)
                    
    def _display_tree(self, task_key, task_status, task_list, task_name, tree=None):
         
        if not task_list.has_key(task_key):
            status_dict={'tree':None, 'name':task_name}
            ...
            try:
                newtree=self.task_view.treelist.prepend(tree, [task_name, status, s_time, None, None, None, None, None, None] )
            except:
            ...
            print "END TIME<%s> CODE<%s>" %(task_status.get('task_end'), task_status.get('task_code') )
            if task_status.get('task_end'):
                print "IF IF IF -------"
            ...
            elif task_status.get('cmd_str') or task_status.get('cmd_desc'):
                self._get_cmd_desc(tree, task_status)
            
            elif task_status.get('cmd_time'):
                self._get_cmd_time(tree, task_status)
                
            elif task_status.get('ack_time'):
                self._get_ack_info(tree, task_status)
                
            elif task_status.get('end_time'):
                self._get_end_time(tree, task_status) 
                
            
            
            #if task_list[task_key]['name'] == 'Native':
#                
#            if task_status['cmd_str'] and task_status['cmd_desc']:
#                self._get_cmd_desc(tree, task_status)
#            elif task_status['cmd_time']:
#                pass
#            elif task_status['ack_time']:
#                pass
#            elif task_status['end_time']:
#                pass
            #self._get_cmd_desc(tree, task_status, 'cmd_str', 0)
            #self._get_cmd_desc(tree, task_status, 'cmd_desc', 4)
...
if __name__ == '__main__':
    ...
    # Can't import GUI widget stuff until DISPLAY is set
    try:
        import pygtk
        pygtk.require("2.0")
    except:
