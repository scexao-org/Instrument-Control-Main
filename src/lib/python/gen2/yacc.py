...
# Default logger to be used if not overridden
logger = logging.getLogger('ply.yacc')
...
##           else: return self.stack[n].value
##        except Exception, e:
##           print "BAD ACCESS: n=%d len slice=%d" % (
##              n, len(self.slice))
##           #for i in xrange(n):
##           #   print "%d: %s" % (i, self.slice[i].value)
##           return "*ERROR*"
...
class Parser:
    def __init__(self,magic=None,logger=None):
    ...
    def parse(self,input=None,lexer=None,debug=0,tracking=0):
        ...
        while 1:
            ...
            if debug > 1:
                self.logger.debug('action %s' % str(t))
            if t is not None:
                if t > 0:
                    # shift a symbol on the stack
                    if ltype == '$end':
                        # Error, end of input
                        self.logger.error("yacc: Parse error. EOF")
                        return
                    statestack.append(t)
                    state = t
                    if debug > 1:
                        self.logger.debug("%-60s shift state %s" % (errorlead, t))
                ...
                if t < 0:
                    ...
                    sym.value = None
                    if debug > 1:
                        self.logger.debug("%-60s reduce %d" % (errorlead, -t))
                ...
                if errorcount == 0 or self.errorok:
                    ...
                    if self.errorfunc:
                        ...
                        if errtoken:
                            ...
                            if lineno:
                                self.logger.error("yacc: Syntax error at line %d, token=%s" % (lineno, errtoken.type))
                            else:
                                self.logger.error("yacc: Syntax error, token=%s" % errtoken.type)
                        else:
                            self.logger.error("yacc: Parse error in input. EOF")
                            return
    ...
    for l in lines:
        ...
        if m:
            ...
            if not prev:
                ...
                logger.warn("%s:%d: Function %s redefined. Previously defined on line %d" % (filename,linen,name,prev))
                noerror = 0
        linen += 1
    return noerror
...
def validate_dict(d):
    for n,v in d.items(): 
        ...
        if n[0:2] == 'p_':
            logger.warn("yacc: Warning. '%s' not defined as a function" % n)
        if 1 and isinstance(v,types.FunctionType) and v.func_code.co_argcount == 1:
            try:
                doc = v.__doc__.split(" ")
                if doc[1] == ':':
                    logger.warn("%s:%d: Warning. Possible grammar rule '%s' defined without p_ prefix." % (v.func_code.co_filename, v.func_code.co_firstlineno,n))
...
def add_production(f,file,line,prodname,syms):
    ...
    if Terminals.has_key(prodname):
        logger.error("%s:%d: Illegal rule name '%s'. Already defined as a token." % (file,line,prodname))
        return -1
    if prodname == 'error':
        logger.error("%s:%d: Illegal rule name '%s'. error is a reserved word." % (file,line,prodname))
        return -1
                
    if not _is_identifier.match(prodname):
        logger.error("%s:%d: Illegal rule name '%s'" % (file,line,prodname))
        return -1
    ...
    for x in range(len(syms)):
        ...
        if s[0] in "'\"":
             try:
                 c = eval(s)
                 if (len(c) > 1):
                      logger.error("%s:%d: Literal token %s in rule '%s' may only be a single character" % (file,line,s, prodname)) 
                      return -1
        ...
        if not _is_identifier.match(s) and s != '%prec':
            logger.error("%s:%d: Illegal name '%s' in rule '%s'" % (file,line,s, prodname))
            return -1
    ...
    if Prodmap.has_key(map):
        m = Prodmap[map]
        logger.error("%s:%d: Duplicate rule %s." % (file,line, m))
        logger.error("%s:%d: Previous definition at %s:%d" % (file,line, m.file, m.line))
        return -1
    ...
    while i < len(p.prod):
        ...
        if t == '%prec':
            try:
                ...
                logger.error("%s:%d: Syntax error. Nothing follows %%prec." % (p.file,p.line))
                return -1

            prec = Precedence.get(precname,None)
            if not prec:
                logger.error("%s:%d: Nothing known about the precedence of '%s'" % (p.file,p.line,precname))
                return -1
            else:
                p.prec = prec
            del p.prod[i]
...
def add_function(f):
    ...
    if f.func_code.co_argcount > reqdargs:
        logger.error("%s:%d: Rule '%s' has too many arguments." % (file,line,f.__name__))
        return -1

    if f.func_code.co_argcount < reqdargs:
        logger.error("%s:%d: Rule '%s' requires an argument." % (file,line,f.__name__))
        return -1
        ...
        for ps in pstrings:
            ...
            try:
                if p[0] == '|':
                    ...
                    if not lastp:
                        logger.error("%s:%d: Misplaced '|'." % (file,dline))
                        return -1
                    prodname = lastp
                    if len(p) > 1:
                        syms = p[1:]
                    ...
                    if assign != ':' and assign != '::=':
                        logger.error("%s:%d: Syntax error. Expected ':'" % (file,dline))
                        return -1
                ...
                logger.error("%s:%d: Syntax error in rule '%s'" % (file,dline,ps))
                error -= 1
    else:
        logger.error("%s:%d: No documentation string specified in function '%s'" % (file,line,f.__name__))
    return error
...
def compute_reachable():
    ...
    for s in Nonterminals.keys():
        if not Reachable[s]:
            logger.warn("yacc: Symbol '%s' is unreachable." % s)
    ...
    for (s,terminates) in Terminates.items():
        if not terminates:
            if not Prodnames.has_key(s) and not Terminals.has_key(s) and s != 'error':
                ...
                logger.error("yacc: Infinite recursion detected for symbol '%s'." % s)
                some_error = 1
...
def verify_productions(cycle_check=1):
    ...
    for p in Productions:
        ...
        for s in p.prod:
            if not Prodnames.has_key(s) and not Terminals.has_key(s) and s != 'error':
                logger.warn("%s:%d: Symbol '%s' used, but not defined as a token or a rule." % (p.file,p.line,s))
                error = 1
                continue
    ...
    for s,v in Terminals.items():
        if s != 'error' and not v:
            logger.warn("yacc: Warning. Token '%s' defined, but not used." % s)
    ...
    for s,v in Nonterminals.items():
        if not v:
            p = Prodnames[s][0]
            logger.warn("%s:%d: Warning. Rule '%s' defined, but not used." % (p.file,p.line, s))
            unused_prod += 1

    
    if unused_tok == 1:
        logger.warn("yacc: Warning. There is 1 unused token.")
    if unused_tok > 1:
        logger.warn("yacc: Warning. There are %d unused tokens." % unused_tok)

    if unused_prod == 1:
        logger.warn("yacc: Warning. There is 1 unused rule.")
    if unused_prod > 1:
        logger.warn("yacc: Warning. There are %d unused rules." % unused_prod)
    ...
    for p in plist:
        ...
        try:
            ...
            if prec != 'left' and prec != 'right' and prec != 'nonassoc':
                logger.error("yacc: Invalid precedence '%s'" % prec)
                return -1
            for t in terms:
                if Precedence.has_key(t):
                    logger.error("yacc: Precedence already specified for terminal '%s'" % t)
                    error += 1
                    continue
                Precedence[t] = (prec,plevel)
        except:
            logger.error("yacc: Invalid precedence table.")
            error += 1
...
def lr_parse_table(method):
    ...
    if yaccdebug:
        logger.debug("yacc: Generating %s parsing table..." % method)
        _vf.write("\n\nParsing method: %s\n\n" % method)
                            ...
                            if r is not None:
                                ...
                                if r > 0:
                                    ...
                                    if oldp.line > pp.line:
                                        st_action[a] = -p.number
                                        st_actionp[a] = p
                                    # logger.warn("Reduce/reduce conflict in state %d" % st)
                                    n_rrconflict += 1
                                    ...
                                    logger.error("Unknown conflict in state %d" % st)
                            else:
                                st_action[a] = -p.number
                                st_actionp[a] = p
                else:
                    ...
                    if Terminals.has_key(a):
                        ...
                        if j >= 0:
                            ...
                            if r is not None:
                                ...
                                if r > 0:
                                    if r != j:
                                        logger.warn("Shift/shift conflict in state %d" % st)
                                elif r < 0:
    ...
    if yaccdebug:
        if n_srconflict == 1:
            logger.debug("yacc: %d shift/reduce conflict" % n_srconflict)
        if n_srconflict > 1:
            logger.debug("yacc: %d shift/reduce conflicts" % n_srconflict)
        if n_rrconflict == 1:
            logger.debug("yacc: %d reduce/reduce conflict" % n_rrconflict)
        if n_rrconflict > 1:
            logger.debug("yacc: %d reduce/reduce conflicts" % n_rrconflict)
...
""")
        ...
        logger.error("Unable to create '%s'" % filename)
        logger.error(str(e))
        return
...
def yacc(method=default_lr, debug=yaccdebug, module=None, tabmodule=tab_module,
         start=None, check_recursion=1, optimize=0,write_tables=1,
         debugfile=debug_file, outputdir='', logger=None):
    ...
    if (optimize and lr_read_tables(tabmodule,1)):
        ...
        if requires:
            ...
            for r,v in requires.items():
                try:
                    ...
                    logger.error("Invalid specification for rule '%s' in require. Expected a list of strings" % r)
        ...
        if 'error' in tokens:
            logger.error("yacc: Illegal token 'error'.  Is a reserved word.")
            raise YaccError,"Illegal token name"

        for n in tokens:
            if Terminals.has_key(n):
                logger.warn("yacc: Warning. Token '%s' multiply defined." % n)
            Terminals[n] = [ ]
        ...
        if ef:
            ...
            logger.warn("yacc: Warning. no p_error() function is defined.")
        ...
        for f in symbols:
            if (add_function(f)) < 0:
                logger.error("Unable to add function '%s'" % f.__name__)
                error += 1
            else:
        ...
        if not lr_read_tables(tabmodule):
            ...
            for filename in files.keys():
                if not validate_file(filename):
                    logger.error("Filename validation failed: '%s'" % (
                          filename))
                    error = 1
            ...
            if error:
               logger.error("verify_productions failed")
            ...
            if yaccdebug:
                try:
                    ...
                    logger.error("yacc: can't create '%s'" % debugfile,e)
        
    # Made it here.   Create a parser object and set up its internal state.
    # Set global parse() method to bound method of parser object.

    p = Parser("xyzzy", logger=logger)
