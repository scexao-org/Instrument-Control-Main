...
def fit_TwoD_Gaussian(img, xc, yc, rad, circ=False, fixrad=False):
    ...
    if fixrad:
        model_init = models.Gaussian2D(amplitude=np.max(img),
                                       x_mean=xc,
                                       y_mean=yc,
                                       x_stddev=rad,
                                       y_stddev=rad,
...
def get_numerical_PSF(file_aperture='pupil.fits',
                      mas_pix=16.2,
                      rotation_angle=-7.,
                      wavelength=1.6e-6):
    import hcipy as hci
...
def calculate_strehl(image,
    ...
    model_init = SubaruPSF(amplitude=np.max(imgstr1),
                           x_0=posce[1],
                           y_0=posce[0])
    ...
    imgstr2 = shift(image, (-yoff, -xoff))[cyi - ca:cyi + ca,
                                           cxi - ca:cxi + ca]
    ...
    if saveplot:
        fig, ax = plt.subplots()
        plt.imshow(np.log10(np.abs(imgstr3)),
                   cmap="inferno",
...
def binary_processing(im,
    ...
    if trackcen:
        model_init = SubaruPSF(amplitude=fluxtmp[0],
                               y_0=postmp[0, 0],
                               x_0=postmp[0, 1])
    x, y = np.mgrid[:ny, :nx]
    ...
    if retroinj:
        ...
        model_init_retro = RetroinjPSF(amplitude=maskedim.max(),
                                       x_0=retrotmp[0],
                                       y_0=retrotmp[1],
                                       bounds={
                                           'x_0': (retrotmp[0] - 20,
    ...
    for i in range(nst - 1):
        ...
        if retroinj and trackstar > 0 and i + 1 == trackstar:
            ...
            postmp[i + int(retroinj) + 1, :] = np.unravel_index(
                indmaxco, (ny, nx))
        ...
        if i == 0 and not trackcen and not retroinj:
            model_init = SubaruPSF(amplitude=fluxtmp[i + int(retroinj) + 1],
                                   x_0=postmp[i + int(retroinj) + 1, 1],
                                   y_0=postmp[i + int(retroinj) + 1, 0],
                                   bounds={
                                       'x_0':
    ...
    angleco = -np.rad2deg(
        np.arctan((posst[1:, 1] - posst[0, 1]) / (posst[1:, 0] - posst[0, 0])))
    ...
    if saveplot:
        ...
        fig, ax = plt.subplots()
        plt.imshow(np.log10(np.abs(im[1:, 1:] / np.max(im))),
                   cmap="inferno",
        ...
        plt.text(cny - rn2 * m.cos(np.deg2rad(pad)),
                 cnx - rn2 * m.sin(np.deg2rad(pad)),
                 "N",
                 color="yellow",
                 ha="center",
                 va="center")
        plt.text(cny - rn2 * m.sin(np.deg2rad(pad)),
                 cnx + rn2 * m.cos(np.deg2rad(pad)),
                 "E",
                 color="yellow",
                 ha="center",
                 va="center")
        ...
        fig, ax = plt.subplots()
        plt.imshow(np.log10(
            np.abs(psf_fit(x, y)[1:, 1:] / np.max(psf_fit(x, y)))),
                   cmap="inferno",
                   extent=[(-cx + 0.5) * as_pix, (cx - 0.5) * as_pix,
                           (cy - 0.5) * as_pix, (-cy + 0.5) * as_pix])
        plt.clim([-4, 0])
    ...
    if verbose:
        ...
        txtfile = open(
            "%s%s_Binary_%s_log.txt" % (savepath, timestamp, target), "w")
        txtfile.write(txt)
        txtfile.close()
