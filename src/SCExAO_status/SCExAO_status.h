#ifndef _SCExAO_status
#define _SCExAO_status

int main(int argc, char **argv);


typedef struct
{

  // DOFs
  char src_fib_st[16];
  int src_fib_co;
  float src_fib_x;
  float src_fib_y;
  char intsphere[16];
  int intsphere_co;
  char oap1_st[16];
  int oap1_co;
  float oap1_theta;
  float oap1_phi;
  int oap1_f;
  char polarizer[16];
  float polarizer_theta;
  int polarizer_co;
  char dichroic_st[16];
  int dichroic_co;
  float dichroic;
  char pupil_st[16];
  int pupil_co;
  float pupil_wheel;
  int pupil_x;
  int pupil_y;
  char compplate[16];
  int compplate_co;
  char piaa1_st[16];
  int piaa1_co;
  float piaa1_wheel;
  int piaa1_x;
  int piaa1_y;
  char piaa2_st[16];
  int piaa2_co;
  float piaa2_wheel;
  int piaa2_x;
  int piaa2_y;
  int piaa2_f;
  char photonics_pickoff_st[16];
  int photonics_pickoff_co;
  float photonics_pickoff;
  char photonics_pickoff_type[16];
  int photonics_pickoff_type_co;
  char photonics_compplate[16];
  int photonics_compplate_co;
  char PG1_pickoff[16];
  int PG1_pickoff_co;
  char fpm_st[16];
  int fpm_co;
  float fpm_wheel;
  int fpm_x;
  int fpm_y;
  int fpm_f;
  char lyot_st[16];
  int lyot_co;
  float lyot_wheel;
  int lyot_x;
  int lyot_y;
  char invpiaa_st[16];
  int invpiaa_co;
  float invpiaa_x;
  float invpiaa_y;
  int invpiaa_theta;
  int invpiaa_phi;
  char oap4_st[16];
  int oap4_co;
  float oap4_theta;
  float oap4_phi;
  char field_stop_st[16];
  int field_stop_co;
  float field_stop_x;
  float field_stop_y;
  char steering_st[16];
  int steering_co;
  float steering_theta;
  float steering_phi;
  char charis_pickoff_st[16];
  int charis_pickoff_co;
  float charis_pickoff_wheel;
  float charis_pickoff_theta;
  char charis_wollaston[16];
  int charis_wollaston_co;
  char mkids_pickoff_st[16];
  int mkids_pickoff_co;
  float mkids_pickoff_wheel;
  float mkids_pickoff_theta;
  char ircam_flc_st[16];
  float ircam_flc;
  int ircam_flc_co;
  char ircam_pupil_st[16];
  int ircam_pupil_co;
  float ircam_pupil_x;
  float ircam_pupil_y;
  char ircam_qwp[16];
  int ircam_qwp_co;
  char ircam_hwp[16];
  int ircam_hwp_co;
  int ircam_hwp_theta;
  char ircam_filter[16];
  int ircam_filter_co;
  char ircam_wollaston[16];
  int ircam_wollaston_co;
  char ircam_block[16];
  int ircam_block_co;
  char ircam_fcs_st[16];
  int ircam_fcs_co;
  int ircam_fcs_f1;
  float ircam_fcs_f2;
  char apapane_pickoff_st[16];
  int apapane_pickoff_co;
  float apapane_pickoff;
  char palila_pup[16];
  int palila_pup_co;
  char palila_pup_fcs_st[16];
  int palila_pup_fcs_co;
  float palila_pup_fcs;
  char irspectro_mode_st[16];
  int irspectro_mode_co;
  int irspectro_mode_x1;
  float irspectro_mode_x2;
  char irspectro_col_st[16];
  int irspectro_col_co;
  float irspectro_col;
  char irspectro_fib_st[16];
  int irspectro_fib_co;
  float irspectro_fib_x;
  float irspectro_fib_y;
  char fibinj_pickoff_st[16];
  int fibinj_pickoff_co;
  float fibinj_pickoff;
  char fibinj_len_st[16];
  int fibinj_len_co;
  float fibinj_len;
  char fibinj_fib_st[16];
  int fibinj_fib_co;
  int fibinj_fib_x;
  int fibinj_fib_y;
  int fibinj_fib_f;
  int fibinj_car;
  char lowfs_block[16];
  int lowfs_block_co;
  int lowfs_fcs;
  char reach_pickoff_st[16];
  int reach_pickoff_co;
  float reach_pickoff;
  char reach_oap_st[16];
  int reach_oap_co;
  float reach_oap_theta;
  float reach_oap_phi;
  char reach_fib_st[16];
  int reach_fib_co;
  int reach_fib_x;
  int reach_fib_y;
  int reach_fib_f;
  float reach_fib_theta;
  char pywfs_pickoff_st[16];
  int pywfs_pickoff_co;
  float pywfs_pickoff;
  int pywfs_fieldstop_co;
  char pywfs_fieldstop_st[16];
  float pywfs_fieldstop_x;
  float pywfs_fieldstop_y;
  char pywfs_filter[16];
  int pywfs_filter_co;
  int pywfs_col;
  int pywfs_fcs;
  char pywfs_fcs_pickoff[16];
  int pywfs_fcs_pickoff_co;
  int pywfs_pup_x;
  int pywfs_pup_y;
  int vampires_fieldstop_co;
  char vampires_fieldstop_st[16];
  float vampires_fieldstop_x;
  float vampires_fieldstop_y;
  char first_pickoff_st[16];
  int first_pickoff_co;
  float first_pickoff;
  char rhea_pickoff_st[16];
  int rhea_pickoff_co;
  float rhea_pickoff;
  int first_inj_co;
  char first_inj_st[16];
  int first_inj_x;
  int first_inj_y;
  float first_inj_f;
  int first_src_co;
  char first_src_st[16];
  float first_src_x;
  float first_src_y;
  char first_photometry_st[16];
  int first_photometry_co;
  float first_photometry;
  char src_select_st[16];
  int src_select_co;
  float src_select;
  char src_flux_irnd[16];
  char src_flux_optnd[16];
  char src_flux_filter[16];

  // WFS
  char pywfs_loop[16];
  int pywfs_loop_co;
  char pywfs_cal[16];
  int pywfs_cal_co;
  int pywfs_freq;
  float pywfs_gain;
  float pywfs_leak;
  float pywfs_rad;
  char pywfs_puploop[16];
  int pywfs_puploop_co;
  char pywfs_cenloop[16];
  int pywfs_cenloop_co;
  char dmoffload[16];
  int dmoffload_co;
  char lowfs_loop[16];
  int lowfs_loop_co;
  int lowfs_freq;
  int lowfs_nmodes;
  char lowfs_mtype[16];
  float lowfs_gain;
  float lowfs_leak;
  char sn_loop[16];
  int sn_loop_co;
  int sn_freq;
  float sn_gain;
  char zap_loop[16];
  int zap_loop_co;
  int zap_freq;
  float zap_gain;
  int zap_nmodes;
  char zap_mtype[16];

  // LOGGING STATUS
  char logpalila[16];
  int logpalila_co;
  char darkpalila[16];
  int darkpalila_co;
  char hotspot[16];
  int hotspot_co;

  // ASTROGRID
  char grid_st[16];
  int grid_st_co;
  float grid_sep;
  float grid_amp;
  int grid_mod;

  // NPS
  char nps1_1[16];
  int nps1_1_co;
  char nps1_2[16];
  int nps1_2_co;
  char nps1_3[16];
  int nps1_3_co;
  char nps1_4[16];
  int nps1_4_co;
  char nps1_5[16];
  int nps1_5_co;
  char nps1_6[16];
  int nps1_6_co;
  char nps1_7[16];
  int nps1_7_co;
  char nps1_8[16];
  int nps1_8_co;
  char nps2_1[16];
  int nps2_1_co;
  char nps2_2[16];
  int nps2_2_co;
  char nps2_3[16];
  int nps2_3_co;
  char nps2_4[16];
  int nps2_4_co;
  char nps2_5[16];
  int nps2_5_co;
  char nps2_6[16];
  int nps2_6_co;
  char nps2_7[16];
  int nps2_7_co;
  char nps2_8[16];
  int nps2_8_co;
  char nps3_1[16];
  int nps3_1_co;
  char nps3_2[16];
  int nps3_2_co;
  char nps3_3[16];
  int nps3_3_co;
  char nps3_4[16];
  int nps3_4_co;
  char nps3_5[16];
  int nps3_5_co;
  char nps3_6[16];
  int nps3_6_co;
  char nps3_7[16];
  int nps3_7_co;
  char nps3_8[16];
  int nps3_8_co;
} SCEXAOSTATUS;

// EXTENSION (Jan 2022) in another struct not to break legacy
typedef struct
{
  char nps2_9[16];
  int nps2_9_co;
  char nps2_10[16];
  int nps2_10_co;
  char nps2_11[16];
  int nps2_11_co;
  char nps2_12[16];
  int nps2_12_co;
  char nps2_13[16];
  int nps2_13_co;
  char nps2_14[16];
  int nps2_14_co;
  char nps2_15[16];
  int nps2_15_co;
  char nps2_16[16];
  int nps2_16_co;
  char logapapane[16];
  int logapapane_co;
} SCEXAOSTATUS_EXTENSION;

#endif // _SCExAO_status
