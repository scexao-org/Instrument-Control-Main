        ...
        if event.type == QUIT:
            ...
            if event.key == K_s:
                ...
                if (mmods & KMOD_LCTRL):
                    ...
                    if saveim:
                        timestamp = dt.datetime.utcnow().strftime('%Y%m%d')
                        if (mmods & KMOD_LSHIFT):
                            savepath = '/media/data/ARCHIVED_DATA/' + timestamp + '/ircam%dlog/' % (camid, )
                        else:
                ...
                if (mmods & KMOD_LCTRL):
                    if (mmods & KMOD_LALT):
                        if xsizeim != 640 and ysizeim != 512:
                            ...
                            time.sleep(2)
                            tmux("scrop_cols 0 639",
                                 session="ircam%dctrl" % (camid, ))
                            time.sleep(2)
                            tmux("scrop_rows 0 511",
                                 session="ircam%dctrl" % (camid, ))
                            time.sleep(2)
            ...
            if event.key == K_MINUS:
                ...
                if (mmods & KMOD_LCTRL):
                    if (mmods & KMOD_LALT):
                        cam_paused.set_data(np.ones(1, dtype=np.float32))
                        tmux("setcrop 10", session="ircam%dctrl" % (camid, ))
                        time.sleep(12)
                        cam_paused.set_data(np.zeros(1, dtype=np.float32))
                        shmreload = True
            ...
            if event.key == K_EQUALS:
                ...
                if (mmods & KMOD_LCTRL):
                    if (mmods & KMOD_LALT):
                        cam_paused.set_data(np.ones(1, dtype=np.float32))
                        tmux("setcrop 11", session="ircam%dctrl" % (camid, ))
                        time.sleep(12)
                        cam_paused.set_data(np.zeros(1, dtype=np.float32))
                        shmreload = True
