...
class myGUI:
    ...
    def __init__(self,):
        ...
        self.axis.set_ylabel('Tip-Tilt moving RMS (micron)')
        self.axis.set_ylim([-80.0, 80.0])
        self.axis.set_ylim([-0.1, 0.1])
        self.axis.grid(True)
        x = np.arange(100)
        self.axis.plot(x)
        self.axis.plot(x)
...
def plot_loop(GUI):
    ...
    while GUI.live_plot:
        ...
        myarr   = np.fromstring(buf1[200:200+4*xs],dtype=np.float32)
        tip   = myarr[0]
        tilt  = myarr[1]
        foc   = myarr[2]
