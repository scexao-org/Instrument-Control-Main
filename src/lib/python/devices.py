    ...
    def __init__(self,
        ...
        if args == []:
            self.usage()
              ...
              in conexnames) or (conu == 0) and args[0].lower() not in defpos:
            if na > conu:
                if conu == 0:
        ...
        elif ((args[0].lower() in zabernames) or
              (zabu
               == 0)) and (zaberchain != "") and args[0].lower() not in defpos:
            if na > zabu:
                if zabu == 0:
            ...
            if self.color_st:
                exec(
                    "subprocess.call(['scxkw-setter', 'set', devname+'_st', self.param1[inddef], self.param%d[inddef]])"
                    % (self.nend, ), globals(), locals())
            else:
                subprocess.call([
                    "scxkw-setter", "set", devname + "_st", self.param1[inddef]
                ])
            ...
            if conexnames != []:
                for i in range(len(conexnames)):
                    ...
                    if not opened:
                        subprocess.call([
                            'scxkw-setter', 'set', self.devname + '_st',
                            'NOT CONNECTED', '0'
                        ])
                        sys.exit()
            ...
            if zabernames != []:
                for i in range(len(zabernames)):
                    ...
                    pos[self.col - 2] = self.zab.status(
                        self.zaberid, self.devnamez)
                    self.col += 1
                    self.zab.close()
            for i in range(self.nslots):
                for j in range(self.nbdev):
                    exec("paramf[j] = float(self.param%d[i])" % (j + 2, ),
                         globals(), locals())
                if (np.sum(paramf[1:]) > 0 and np.all(pos == paramf)) or (
                        np.sum(paramf[1:]) == 0 and pos[0] == paramf[0]):
                    print("Device is in position " + self.param0[i] + ", " +
                          self.param1[i])
                    if self.color_st:
                        exec(
                            "subprocess.call(['scxkw-setter', 'set', self.devname+'_st', self.param1[i][:16], self.param%d[i]])"
                            % (self.nend, ), globals(), locals())
                    else:
                        subprocess.call([
                            "scxkw-setter", "set", self.devname + "_st",
            ...
            if not found:
                ...
                subprocess.call([
                    'scxkw-setter', 'set', self.devname + '_st', 'UNKNOWN', '3'
                ])
            ...
            if self.conexnames != []:
                for i in range(len(self.conexnames)):
                    print("    %-6s  move %s stage" %
                          (self.conexnames[i], self.conexnames[i]))
            if self.zabernames != []:
                for i in range(len(self.zabernames)):
                    print("    %-6s  move %s stage" %
                          (self.zabernames[i], self.zabernames[i]))
     ...
     1 - %d defined positions""" % (self.nslots, ))
        if self.defpos != []:
            for i in range(len(self.defpos)):
                print("    %-6s  move stage to %s position" %
                      (self.defpos[i], self.defpos[i]))
        print("""ARG:
...
CONTENT:""")
    ...
    def conex_home(self, conu):
        ...
        if not opened:
            subprocess.call([
                'scxkw-setter', 'set', self.devname + '_st', 'NOT CONNECTED',
                '0'
            ])
            sys.exit()
        ...
        pos0 = -1000.
        d = locals()
        exec("paramf = list(map(float, self.param%d))" % (self.col, ),
             globals(), d)
        paramf = d['paramf']
        ...
        while pos0 != pos:
            ...
            pos = self.con.status(self.devnamec)
            time.sleep(0.2)
            subprocess.call(["scxkw-setter", "set", self.devnamec, str(pos)])
            if conu == 0:
                if pos in paramf:
                    for i in range(self.nslots):
                        if pos == paramf[i]:
                            if self.color_st:
                                exec(
                                    "subprocess.call(['scxkw-setter', 'set', self.devnamec+'_st', self.param1[i][:16], self.param%d[i]])"
                                    % (self.nend, ), globals(), locals())
                            else:
                                subprocess.call([
                                    "scxkw-setter", "set",
                                    self.devnamec + "_st", self.param1[i][:16]
                                ])
                else:
                    subprocess.call([
                        'scxkw-setter', 'set', self.devnamec + '_st',
                        'UNKNOWN', '3'
                    ])
        self.con.close()
    ...
    def conex_status(self, conu):
        ...
        if pos in paramf:
            for i in range(self.nslots):
                if pos == paramf[i]:
                    print("Position = " + str(pos) +
                          ", Conex is in position " + self.param0[i] + ", " +
                          self.param1[i])
                    if conu == 0:
                        if self.color_st:
                            exec(
            ...
            print("Position = " + str(pos) +
                  ", Conex is not in a defined position. Try homing.")
            if conu == 0:
                subprocess.call([
                    'scxkw-setter', 'set', self.devnamec + '_st', 'UNKNOWN',
                    '3'
                ])
    ...
    def conex_goto(self, pos, conu):
        ...
        pos = self.con.status(self.devnamec)
        subprocess.call(
            ['scxkw-setter', 'set', self.devnamec + '_st', 'UNKNOWN', '3'])
        while pos0 != pos:
            pos0 = pos
            pos = self.con.status(self.devnamec)
            time.sleep(0.1)
    ...
    def conex_goto_slot(self, slot, conu):
        if (1 <= slot <= self.nslots):
            ...
            while pos0 != pos:
                ...
                time.sleep(0.2)
                subprocess.call(
                    ["scxkw-setter", "set", self.devnamec,
                     str(pos)])
                if conu == 0:
                    if pos in paramf:
                        for i in range(self.nslots):
    ...
    def zaber_home(self, zabu):
        ...
        while pos0 != pos:
            ...
            pos = self.zab.status(self.zaberid, self.devnamez)
            time.sleep(0.2)
            subprocess.call(["scxkw-setter", "set", self.devnamez, str(pos)])
            if zabu == 0:
                if pos in paramf:
                    for i in range(self.nslots):
                        if pos == paramf[i]:
                            if self.color_st:
                                exec(
                                    "subprocess.call(['scxkw-setter', 'set', self.devnamez+'_st', self.param1[i][:16], self.param%d[i]])"
                                    % (self.nend, ), globals(), locals())
                            else:
                                subprocess.call([
                                    "scxkw-setter", "set",
                                    self.devnamez + "_st", self.param1[i][:16]
                                ])
                else:
                    subprocess.call([
                        'scxkw-setter', 'set', self.devnamez + '_st',
                        'UNKNOWN', '3'
                    ])
        self.zab.close()
    ...
    def zaber_status(self, zabu):
        ...
        if pos == 0:
            print("Zaber is home.")
        d = locals()
        exec("paramf = list(map(int, self.param%d))" % (self.col, ), globals(),
             d)
        paramf = d['paramf']
        if pos in paramf:
            for i in range(self.nslots):
                if pos == paramf[i]:
                    print("Position = " + str(pos) +
                          ", Zaber is in position " + self.param0[i] + ", " +
                          self.param1[i])
                    if zabu == 0:
                        if self.color_st:
                            exec(
            ...
            print("Position = " + str(pos) +
                  ", Zaber is not in a defined position. Try homing.")
            if zabu == 0:
                subprocess.call([
                    'scxkw-setter', 'set', self.devnamez + '_st', 'UNKNOWN',
                    '3'
                ])
        ...
        if (1 <= slot <= self.nslots):
            ...
            if int(pos) > 0:
                ...
                while pos0 != pos:
                    ...
                    time.sleep(0.2)
                    subprocess.call(
                        ["scxkw-setter", "set", self.devnamez,
                         str(pos)])
                    if zabu == 0:
                        if pos in paramf:
                            for i in range(self.nslots):
