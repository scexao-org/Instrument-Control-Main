/* @(#)iqefunc.c	14.3 (ESO-DMD) 04/27/00 11:25:31 */
/*===========================================================================
  ...
  for (m=0; m<5; m++) {
	...
	if (0.0<*pw && fabs(val-ba)<fks) {
	   bm += val; bs += val*val; mt++;
            ...
            /*
	     * make sure we clipped the dark pixel.
	     * bright pixel clipping does not 
	     * count as 'caught' here.
	     */ 
	   if (ba-val > fks){
	     clipped_dark = 1;
	   }else{
      ...
      /*
      * decrement kappa if all of the following condition applies.
      * 1) no "dark" pixel was caught by the previous kappa-sigma iteration
      * 2) the standard deviation is larger than 20% of the mean value.
      * 3) kappa is > 3.5   
       */
     if (!(clipped_bright && clipped_dark) 
                     && kappa > 3.5 ){
     	float tmp_ba=0, tmp_bs=0;
     	tmp_ba = bm/mt; tmp_bs = bs/mt - tmp_ba*tmp_ba;
     	tmp_bs = (0.0<tmp_bs) ? sqrt(tmp_bs) : 0.0;
       if(tmp_bs/tmp_ba > 0.20){
     	    kappa = kappa - 0.5;
     	    m = m - 1;
        }
     }
