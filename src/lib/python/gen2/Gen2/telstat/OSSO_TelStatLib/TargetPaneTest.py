...
# TargetPaneTest.py -- Arne Grimstrup -- 2003-10-01
# copied shameless from PointingPaneTest.py -- Bruce Bon -- 2003-09-29 12:05

# Test program for TargetPane.py
...
def SigHandler(signum, frame):
    ...
    print "TargetPaneTest.SigHandler() called, signum = ",	\
	    signum,".\n"
    EXITCODE = signum
    EndProc()
...
def EndProc():
    """Exit function."""
    print "TargetPaneTest.EndProc() called, EXITCODE = ",	\
	    EXITCODE,".\n"
    root.destroy()
    sys.exit( EXITCODE )
...
def RootRefresh():
    global dbgcnt
    print "TargetPaneTest.RootRefresh() called, time = %.6f" % time.time()
    StatIO_service()
...
root["bg"] = "black"
root.iconname( 'TargetPaneTest' )
root.title( 'TargetPaneTest' )
