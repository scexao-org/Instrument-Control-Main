...
wder = "/media/data/20141113/LLOWFS/llowfs_images/lowfs_"
...
fname1 = "/tmp/aol0_DMmode_cmd1.im.shm"  # residuals
fname2 = "/tmp/aol0_DMmode_cmd.im.shm"   # to be applied

fname3 = "/tmp/aol0_DMmode_AVE.im.shm"
fname4 = "/tmp/aol0_DMmode_LIMIT.im.shm"
fname5 = "/tmp/aol0_DMmode_RMS.im.shm"
fname6 = "/tmp/aol0_DMmode_GAIN.im.shm"

f1 = os.open(fname1, os.O_RDONLY)
b1 = mmap.mmap(f1, 0, mmap.MAP_SHARED, mmap.PROT_READ)

f2 = os.open(fname2, os.O_RDONLY)
b2 = mmap.mmap(f2, 0, mmap.MAP_SHARED, mmap.PROT_READ)

f3 = os.open(fname3, os.O_RDONLY)
b3 = mmap.mmap(f3, 0, mmap.MAP_SHARED, mmap.PROT_READ)

f4 = os.open(fname4, os.O_RDONLY)
b4 = mmap.mmap(f4, 0, mmap.MAP_SHARED, mmap.PROT_READ)

f5 = os.open(fname5, os.O_RDONLY)
b5 = mmap.mmap(f5, 0, mmap.MAP_SHARED, mmap.PROT_READ)

f6 = os.open(fname6, os.O_RDONLY)
b6 = mmap.mmap(f6, 0, mmap.MAP_SHARED, mmap.PROT_READ)
...
fd1  = os.open("/tmp/ircam2.im.shm", os.O_RDWR)
...
buffer_is_full=[0,0]
image_cube=[np.empty((NB_frames, ysize, xsize), dtype='uint32') , np.empty((NB_frames, ysize, xsize), dtype='uint16') ]
time_text=["",""]
        ...
        if wrt==0 and cntr!=cntro:
           ...
           # img_date="%02d-%02d-%02d-%06d"%(t.hour,t.minute,t.second,img_tv_nsec/1000)
            ...
            if cntr ==cntr_end:
                ...
                cntro=cntr          
                time_text[buffer_channel] += img_date+'     '+str(val)+'             '+str(cntr)+'        '+str(resX)+'       '+str(resY)+'    '+str(dmX)+'     '+ str(dmY)+ '    ' +str(AveX)+'    '+str(AveY)+'   '  +str(LimX)+'     ' +str(LimY)+ '       ' +str(rmsX)+  '     '+str(rmsY)+ '    '+ str(GX)+ '    '+ str(GY)+ '   '+str(TGX)+'    '+str(TGY)+'    '+"\n"
