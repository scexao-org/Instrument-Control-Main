...
# Default logger if not overridden
logger = logging.getLogger('ply.lex')
    ...
    for l in lines:
        ...
        if m:
            ...
            if not prev:
                ...
                logger.warn("%s:%d: Rule %s redefined. Previously defined on line %d" % (filename,linen,name,prev))
                noerror = 0
        linen += 1
    return noerror
...
def lex(module=None,object=None,debug=0,optimize=0,lextab="lextab",reflags=0,nowarn=0):
    ...
    if not optimize:
        for n in tokens:
            if not _is_identifier.match(n):
                logger.error("lex: Bad token name '%s'" % n)
                error = 1
            if warn and lexobj.lextokens.has_key(n):
                logger.warn("lex: Warning. Token '%s' multiply defined." % n)
            lexobj.lextokens[n] = None
    else:
        for n in tokens: lexobj.lextokens[n] = None

    if debug:
        logger.debug("lex: tokens = '%s'" % lexobj.lextokens.keys())

    try:
         for c in literals:
               if not (isinstance(c,types.StringType) or isinstance(c,types.UnicodeType)) or len(c) > 1:
                    logger.error("lex: Invalid literal %s. Must be a single character" % repr(c))
                    error = 1
                    continue

    except TypeError:
         logger.error("lex: Invalid literals specification. literals must be a sequence of characters.")
         error = 1
    ...
    if states:
         if not (isinstance(states,types.TupleType) or isinstance(states,types.ListType)):
              logger.error("lex: states must be defined as a tuple or list.")
              error = 1
         else:
              for s in states:
                    if not isinstance(s,types.TupleType) or len(s) != 2:
                           logger.error("lex: invalid state specifier %s. Must be a tuple (statename,'exclusive|inclusive')" % repr(s))
                           error = 1
                           continue
                    name, statetype = s
                    if not isinstance(name,types.StringType):
                           logger.error("lex: state name %s must be a string" % repr(name))
                           error = 1
                           continue
                    if not (statetype == 'inclusive' or statetype == 'exclusive'):
                           logger.error("lex: state type for state %s must be 'inclusive' or 'exclusive'" % name)
                           error = 1
                           continue
                    if stateinfo.has_key(name):
                           logger.error("lex: state '%s' already defined." % name)
                           error = 1
                           continue
    ...
    for f in tsymbols:
        ...
        if callable(t):
            ...
            logger.error("lex: %s not defined as a function or string" % f)
            error = 1
    ...
    for state in stateinfo.keys():
        ...
        for fname, f in funcsym[state]:
            ...
            if not optimize:
                ...
                if tokname == 'ignore':
                    logger.error("%s:%d: Rule '%s' must be defined as a string." % (file,line,f.__name__))
                    error = 1
                    continue
            ...
            if f.__doc__:
                if not optimize:
                    try:
                        ...
                        if c.match(""):
                             logger.error("%s:%d: Regular expression for rule '%s' matches empty string." % (file,line,f.__name__))
                             error = 1
                             continue
                    except re.error,e:
                        logger.error("%s:%d: Invalid regular expression for rule '%s'. %s" % (file,line,f.__name__,e))
                        if '#' in f.__doc__:
                             logger.error("%s:%d. Make sure '#' in rule '%s' is escaped with '\\#'." % (file,line, f.__name__))
                        error = 1
                        continue

                    if debug:
                        logger.debug("lex: Adding rule %s -> '%s' (state '%s')" % (f.__name__,f.__doc__, state))
                ...
                logger.error("%s:%d: No regular expression defined for rule '%s'" % (file,line,f.__name__))
        ...
        for name,r in strsym[state]:
            ...
            if tokname == 'ignore':
                 if "\\" in r:
                      logger.warn("lex: Warning. %s contains a literal backslash '\\'" % name)
                 ignore[state] = r
                 continue
            ...
            if not optimize:
                ...
                if not lexobj.lextokens.has_key(tokname) and tokname.find("ignore_") < 0:
                    logger.error("lex: Rule '%s' defined for an unspecified token %s." % (name,tokname))
                    error = 1
                    continue
                try:
                    c = re.compile("(?P<%s>%s)" % (name,r),re.VERBOSE | reflags)
                    if (c.match("")):
                         logger.error("lex: Regular expression for rule '%s' matches empty string." % name)
                         error = 1
                         continue
                except re.error,e:
                    logger.error("lex: Invalid regular expression for rule '%s'. %s" % (name,e))
                    if '#' in r:
                         logger.error("lex: Make sure '#' in rule '%s' is escaped with '\\#'." % name)

                    error = 1
                    continue
                if debug:
                    logger.debug("lex: Adding rule %s -> '%s' (state '%s')" % (name,r,state))
                
            regex_list.append("(?P<%s>%s)" % (name,r))

        if not regex_list:
             logger.error("lex: No rules defined for state '%s'" % state)
             error = 1
    ...
    for state in regexs.keys():
        ...
        if debug:
            for i in range(len(re_text)):
                 logger.error("lex: state '%s'. regex[%d] = '%s'" % (state, i, re_text[i]))
    ...
    if warn and not lexobj.lexerrorf:
        logger.warn("lex: Warning. no t_error rule is defined.")

    # Check state information for ignore and error rules
    for s,stype in stateinfo.items():
        if stype == 'exclusive':
              if warn and not errorf.has_key(s):
                   logger.warn("lex: Warning. no error rule is defined for exclusive state '%s'" % s)
              if warn and not ignore.has_key(s) and lexobj.lexignore:
                   logger.warn("lex: Warning. no ignore rule is defined for exclusive state '%s'" % s)
