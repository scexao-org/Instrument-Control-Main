...
wder="/media/data/20141109/Science_images/saphira_"
...
fd1  = os.open("/tmp/saphira.im.shm", os.O_RDWR)
        ...
        if wrt==0 and cntr!=cntro:
           ...
           # img_date="%02d-%02d-%02d-%06d"%(t.hour,t.minute,t.second,img_tv_nsec/1000)
            ...
            if cntr ==cntr_end:
                ...
                cntro=cntr          
                time_text[buffer_channel] += img_date+'         '+ str(cntr) +"\n"
...
def save_data():
    ...
    while 1:
        ...
        if buffer_is_full[0]==1 or buffer_is_full[1]==1 :
            ...
            pyfits.writeto(wder+str(la_date)+'_'+s+".fits",image_cube[buffer_channel_full][0:final_index_cube], clobber=True)
        
            f=open(wder+str(la_date)+'_'+ s+".txt",'w')              
            f.write("Date...........UT..........Exposure Time (us)......Frame number \n")         
...
def get_lowfs_img():
    ...
    myarr = np.fromstring(buf1[200:200+4*nel],dtype=np.float32).reshape(ysize, xsize)
